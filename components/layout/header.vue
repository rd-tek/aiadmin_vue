<template>
  ...
  >
        ...
        <ul class="menu">
          ...
          >
            <a
              ...
              :class="{ 'is-active': gnbDepthIndex === i }"
              >{{ item.name }}</a
            >
            ...
            <div class="user-alarm">
              <div class="tooltip tooltip-alarm" ref="tooltipAlarmRef">
                    ...
                    <div class="tooltip-body">
                      <div class="content">
                        <div class="content-list" v-for="(item, index) in notification" :key="index">
                          <p class="content-title">[{{ item.not_type }}]</p>
                          <p class="content-desc" style="cursor: pointer;" @click="_notificationRead(item)">
                            {{ item.not_message }}
                          </p>
                          <p class="content-date">{{common.dateformat(item?.timezone_regdate?.timezone_common)}}</p>
                        </div>
                ...
                >
                  ...
                  <span class="num" v-if="!tooltipShow && notification.length > 0">{{ notification.length }}</span>
    ...
    <div class="mob-menu is-mob">
      <button type="button" class="btn-menu" @click="handleMenu($event)" aria-label="메뉴">
            ...
            <ul class="menu">
              <li class="menu-list" v-for="(item, i) in menuList" :key="i">
                ...
                <transition
                  ...
                  @leave="leave"
                >
                  <div class="slide" v-if="gnbIndex === i && item.children">
                    <ul class="depth">
                      <li
                        class="depth-list"
                        v-for="(depth, j) in item.children"
          ...
          <div class="header">
            <button type="button" class="btn-prev" @click="btnPrev" aria-label="이전">
              <svg xmlns="http://www.w3.org/2000/svg" width="19" height="20" viewBox="0 0 19 20" fill="none">
                <mask id="mask0_27_2531" style="mask-type:alpha" maskUnits="userSpaceOnUse" x="0" y="0" width="19" height="20">
                <rect y="0.789551" width="18.421" height="18.421" fill="#D9D9D9"/>
                </mask>
                <g mask="url(#mask0_27_2531)">
                <path d="M7.6754 17.6755L0 10.0001L7.6754 2.32471L9.03778 3.68709L2.72477 10.0001L9.03778 16.3131L7.6754 17.6755Z" fill="white"/>
                </g>
              </svg>
            </button>
            <button type="button" class="logo" @click="goToMain" aria-label="로고">
          ...
          <div class="content">
            ...
            <ul class="menu">
              <li class="menu-list" v-for="(item, i) in myMenuList" :key="i">
                ...
                >
                  <span class="text">{{ item.name }}</span>
                  <img src="/public/images/icon/icon_arrow_down_black.png" alt="icon_arrow_down_black" v-if="item.children" />
                </a>
              </li>
            </ul>
          </div>
...
watch(
  ...
  () => {
    ...
    if (header) {
      header.classList.remove("is-active", "is-scroll-top");
    }
  }
);
...
const handleScroll = () => {
  ...
  if (scrollTop > lastScrollTop.value) {
    ...
    if (coachingTab) {
      coachingTab.style.top = isMobile ? "60px" : "0px";
    }
  } else {
    isScrolledDown.value = false;
    ...
    const academyLeft = document.querySelector(".academy-left");
    if (academyLeft) {
      academyLeft.style.top = isMobile ? "0" : "97px";
    }
