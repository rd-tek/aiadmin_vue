    ...
    <div class="customer-member detail">
        <div class="detail-wrap" :class="{ 'is-move': detailMove }" ref="detailRef">
            ...
            <div class="page-area">
                <div class="page-area-list">
                    ...
                    <span class="cnt">
                        <nuxt-link
                            v-if="prevPost"
                            :to="`/customer/library/${prevPost.resources_pk}`"
                            >
                            {{ prevPost.title }}
                        </nuxt-link>
                        <span v-if="prevPost == null" class="no-link">
                            이전 글이 없습니다.
                        </span>
                    </span>
                </div>
                <div class="page-area-list">
                    ...
                    <span class="cnt">
                        <nuxt-link
                            v-if="nextPost"
                            :to="`/customer/library/${nextPost.resources_pk}`"
                            >
                            {{ nextPost.title }}
                        </nuxt-link>
                        <span v-if="nextPost == null" class="no-link">
                            다음 글이 없습니다.
                        </span>
                    </span>
                </div>
            ...
            <div class="btn-wrap column-reverse">
                <nuxt-link to="/customer/library" class="btn-md-line">목록보기</nuxt-link>
                  <button
                        type="button"
                        class="btn-md-line"
                        @click="handleDelete"
...
const getNoticeDetail = async () => {
  const res = await customerApi._resourcesView(route.params.slug);
  noticeInfo.value = res.resourcesview || {};
...
const handleEdit = () => {
  router.push(`/customer/library/edit/${route.params.slug}`);
};
...
const handleDelete = async () => {
  ...
  try {
    const res = await customerApi._resourcesDelete(
      route.params.slug,
    );
