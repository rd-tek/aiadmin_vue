          ...
          <div class="table-body-row" v-for="(item, index) in tableList" :key="index" :class="{ 'is-move': tableMove }" ref="tableRef">
            <div class="table-body-flex flex-column">
              ...
              <div class="table-body-col align-left">
                <nuxt-link :to="`/customer/shop/${item.noticeno}`" class="link" :class="{ 'line-through': item.note === '삭제' }">{{ item.title }}</nuxt-link>
              </div>
              <div class="table-body-col col-1 is-mob">
...
const getNoticelist = async () => {
  try {
    const res = await customerApi._ownerNoticelist(searchForm);
    console.log("ownerNoticelist response", res);
...
const handleWrite = () => {
  router.push(`/customer/shop/write`)
}
