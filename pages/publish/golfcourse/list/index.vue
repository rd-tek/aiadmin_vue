<template>
    <div class="golf-course list">
      <div class="filter-box">
        <div class="col-2 flex-end">
            <div class="col d-flex">
                <div class="d-flex">
                    <select>
                        <option>제목</option>
                    </select>
                    <input type="text" v-model="nickname" placeholder="검색어" @keyup.enter="handleSearch">
                </div>
                <button type="button" class="btn" @click="handleSearch">검색</button>
            </div>
        </div>
      </div>
      <div class="row">
        <div class="row-list" v-for="(item, index) in rowList" :key="index">
          <div class="row-list-item col-1">
            <div class="checkbox">
              <div class="check-box">
                <input type="checkbox" :id="`chk_${index}`" :name="`chk_${index}`">
                <label :for="`chk_${index}`">
                  <span class="check-box-item">
                      <i class="item-line" />
                  </span>
                </label>
              </div>
            </div>
          </div>
          <div class="row-list-item">
            <div class="state type01">
              <span class="state-list color-blue" v-if="item.state1 === '사용'">사용</span>
              <span class="state-list border-grey" v-else-if="item.state1 === '대기'">대기</span>
              <span class="state-list color-green" v-if="item.state2 === '공개'">공개</span>
              <span class="state-list color-black" v-else-if="item.state2 === '비공개'">비공개</span>
            </div>
            <div class="title">
              <nuxt-link :to="`/publish/golfcourse/list/${index}`" class="link">{{ item.title }}</nuxt-link>
            </div>
            <div class="label">
              <div class="label-list">{{ item.label }}</div>
            </div>
          </div>
          <div class="row-list-item col-1 align-center">
            <div class="hole">
              {{ item.hole }}홀
            </div>
          </div>
          <div class="row-list-item col-3">
            <div class="diff">
              <div class="diff-list">
                <span class="text">코스난이도</span>
                <div class="star">
                  <span class="star-list" v-for="index in 5" :key="index">
                    <svg xmlns="http://www.w3.org/2000/svg" width="19" height="19" viewBox="0 0 19 19" fill="none">
                      <path d="M18.2961 6.58663C18.2384 6.41682 18.1323 6.26761 17.9908 6.15742C17.8493 6.04723 17.6787 5.98089 17.4999 5.96658L12.2708 5.55107L10.008 0.542056C9.9359 0.380725 9.81869 0.243699 9.67047 0.147514C9.52225 0.0513295 9.34936 9.71158e-05 9.17266 1.37932e-07C8.99597 -9.684e-05 8.82302 0.0509458 8.6747 0.146968C8.52637 0.24299 8.40901 0.379887 8.33677 0.541138L6.07395 5.55107L0.844797 5.96658C0.669105 5.9805 0.501144 6.04474 0.361006 6.15161C0.220868 6.25849 0.114488 6.40348 0.0545932 6.56923C-0.00530196 6.73498 -0.0161763 6.91448 0.0232713 7.08625C0.062719 7.25802 0.150818 7.41479 0.277029 7.5378L4.14134 11.3049L2.77466 17.2229C2.73316 17.402 2.74646 17.5895 2.81283 17.761C2.8792 17.9324 2.99559 18.08 3.14687 18.1845C3.29816 18.2891 3.47737 18.3457 3.66124 18.3471C3.84511 18.3485 4.02516 18.2946 4.17803 18.1924L9.17237 14.8628L14.1667 18.1924C14.3229 18.2961 14.5072 18.3495 14.6947 18.3454C14.8822 18.3413 15.0639 18.2799 15.2155 18.1694C15.367 18.0589 15.4811 17.9047 15.5423 17.7274C15.6036 17.5502 15.6091 17.3584 15.5581 17.1779L13.8805 11.3076L18.0411 7.56348C18.3135 7.31766 18.4135 6.93426 18.2961 6.58663Z" :fill="index < item.course ? '#F8AD15' : '#E5E7E8'"/>
                    </svg>
                  </span>
                </div>
              </div>
              <div class="diff-list">
                <span class="text">그린난이도</span>
                <div class="star">
                  <span class="star-list" v-for="index in 5" :key="index">
                    <svg xmlns="http://www.w3.org/2000/svg" width="19" height="19" viewBox="0 0 19 19" fill="none">
                      <path d="M18.2961 6.58663C18.2384 6.41682 18.1323 6.26761 17.9908 6.15742C17.8493 6.04723 17.6787 5.98089 17.4999 5.96658L12.2708 5.55107L10.008 0.542056C9.9359 0.380725 9.81869 0.243699 9.67047 0.147514C9.52225 0.0513295 9.34936 9.71158e-05 9.17266 1.37932e-07C8.99597 -9.684e-05 8.82302 0.0509458 8.6747 0.146968C8.52637 0.24299 8.40901 0.379887 8.33677 0.541138L6.07395 5.55107L0.844797 5.96658C0.669105 5.9805 0.501144 6.04474 0.361006 6.15161C0.220868 6.25849 0.114488 6.40348 0.0545932 6.56923C-0.00530196 6.73498 -0.0161763 6.91448 0.0232713 7.08625C0.062719 7.25802 0.150818 7.41479 0.277029 7.5378L4.14134 11.3049L2.77466 17.2229C2.73316 17.402 2.74646 17.5895 2.81283 17.761C2.8792 17.9324 2.99559 18.08 3.14687 18.1845C3.29816 18.2891 3.47737 18.3457 3.66124 18.3471C3.84511 18.3485 4.02516 18.2946 4.17803 18.1924L9.17237 14.8628L14.1667 18.1924C14.3229 18.2961 14.5072 18.3495 14.6947 18.3454C14.8822 18.3413 15.0639 18.2799 15.2155 18.1694C15.367 18.0589 15.4811 17.9047 15.5423 17.7274C15.6036 17.5502 15.6091 17.3584 15.5581 17.1779L13.8805 11.3076L18.0411 7.56348C18.3135 7.31766 18.4135 6.93426 18.2961 6.58663Z" :fill="index < item.green ? '#298097' : '#E5E7E8'"/>
                    </svg>
                  </span>
                </div>
              </div>
            </div>
          </div>
        </div>
        <div class="btn-wrap">
          <button type="button" class="btn-md-line">노출</button>
          <button type="button" class="btn-md-fill btn-primary-purple" @click="handleSave">등록</button>
        </div>
      </div>
    </div>
</template>
<script setup>
import { useRouter } from "vue-router";
const router = useRouter();

const rowList = [
  {
    state1: '사용',
    state2: '공개',
    title: 'XGA-Spyglass Hill Golf Course',
    label: 'C_0091',
    hole: 18,
    course: 2,
    green: 5
  },
  {
    state1: '대기',
    state2: '비공개',
    title: 'XGA-Spyglass Hill Golf Course',
    hole: 18,
    course: 1,
    green: 4
  },
  {
    state1: '사용',
    state2: '비공개',
    title: 'XGA-Spyglass Hill Golf Course',
    hole: 18,
    course: 1,
    green: 4
  }
]

const handleSave = () => {
  router.push(`/publish/golfcourse/edit`)
}

// 2026.03.04[cgnoh]: 페이지 메타 정보
definePageMeta({
  layout: "publish-default",
});
</script>
<style lang="scss" scoped>
@use '@/assets/scss/pages/golf-course.scss';
</style>