<template>
    <div class="customer-member">
      ...
      <div class="table type03 mob-type02">
          ...
          <div class="table-body-row" v-for="(item, index) in tableList" :key="index" :class="{ 'is-move': tableMove }" ref="tableRef">
            <div class="table-body-flex flex-column">
              ...
              <div class="table-body-col align-left">
                <nuxt-link :to="`/publish/customer/library/${index}`" class="link" :class="{ 'line-through': item.note === '삭제' }">{{ item.title }}</nuxt-link>
              </div>
              <div class="table-body-col col-1 is-mob">
        ...
        <div class="btn-wrap">
          <button type="button" class="btn-md-fill btn-primary-purple" @click="handleWrite">등록</button>
        </div>
      </div>
    </div>
</template>
...
const handleWrite = () => {
  router.push(`/publish/customer/library/write`)
}
